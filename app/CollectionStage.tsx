...
export default function CollectionStage({ items }: { items: StageItem[]; totalCount: number }) {
  ...
  const colors = useMemo(() => fragmentColors(items.map(item => item.id)), [items])
  ...
  return (
    <main className="collection-page">
        ...
        {items.length === 0 ? <div className="empty-state"><h3>A collection starts with a little noticing.</h3><p>Fragments collected with Spectacles will appear here.</p></div> : (
          <div className="fragment-grid">{items.map((item, index) => (
            <button className="fragment-card" type="button" key={item.id} onClick={() => open(index)} aria-label={`Explore fragment ${index + 1}: ${item.caption}`}>
              <div className="fragment-art" style={{ backgroundColor: colors[index] }}><span className="fragment-number">{String(index + 1).padStart(3, '0')}</span><FragmentImage item={item} eager={index < 4} /><svg className="open-mark" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" aria-hidden="true"><path d="M5 19 19 5M5 5h14v14" /></svg></div>
      ...
      <dialog ref={dialog} className="fragment-dialog" style={{ backgroundColor: colors[selected] }} onClick={event => { if (event.target === event.currentTarget) dialog.current?.close() }} onKeyDown={event => { if (event.key === 'ArrowRight') setSelected(index => (index + 1) % items.length); if (event.key === 'ArrowLeft') setSelected(index => (index - 1 + items.length) % items.length) }} aria-labelledby="fragment-title">
